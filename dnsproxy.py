...
def main():
    ...
    def strand(func, *args, **kwargs):
        ...
        t.start()
        return t
    os.system("sudo networksetup -setdnsservers Wi-Fi 192.168.29.1")
    time.sleep(3)
    dnsserver = DNSProxyServer("8.8.8.8", disable_cache=False, host="127.0.0.1", port=53, hosts_file="/etc/hosts")
    strand(dnsserver.serve_forever)
    time.sleep(4)
    os.system("sudo networksetup -setdnsservers Wi-Fi 127.0.0.1")
    print("done")
